...
|:::image type="icon" source="./media/availability-group-manually-configure-tutorial-single-subnet/square.png" border="false":::   **Network Security Groups (NSGs)** | - SQL Server: **1433** for default instance <br/> If the environment is using [Network security groups](azure-docs/articles/virtual-network/network-security-groups-overview.md), ensure that the current configuration **allows traffic** through ports described on “Firewall ports open” <br/> |
